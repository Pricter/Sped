    ...
    const char* application_name)
{
    ...
    const struct xcb_setup_t *setup = xcd_get_setup(state->connection);
    xcb_screen_iterator_t it = xcb_setup_root_iterator(setup)
    int screen_p = 0;
    ...
    u32 event_mask = XCB_CW_BLACK_PIXEL | XCB_CW_EVENT_MASK;
    ...
    xcb_change_property(
        ...
        XCB_ATOM_WM_NAME,
        XCB_ATOM_STRING, 8
        string_length(application_name),
        application_name);
    ...
    xcb_intern_atom_cookie_t wm_protocols_cookie = xcb_intern_atom(
        ...
        "WM_PROTOCOLS");
    xcb_intern_atom_cookie_t wm_delete_reply = xcb_intern_atom(
        state->connection,
        wm_delete_cookies, NULL);
    xcb_intern_atom_cookie_t wm_protocols_reply = xcb_intern_atom(
        state->connection,
    ...
    if(stream_result <= 0) {
        SFATAL("An error occured when flushing the screen: %d", stream_result);
        return FALSE
    }
...
void platform_console_write(const char* message, u8 color) {
    ...
    printf("\003[%sm%s\033[0m", color_strings[color], message);
}

void platform_console_write_error(const char* message, u8 color) {
    const char* color_strings[] = {"0;41", "1;31", "1;33", "1;32", "1;34", "1;30"};
    fprintf(stderr, "\003[%sm%s\033[0m", color_strings[color], message);
}
...
void platform_get_required_extension_names(const char ***names_darray) {
    darray_push(*names_darray, &"VK_KHR_xcb_surface"); // NOTE: Can use xlib
}
                                  ...
                                  vulkan_context* context)
{
    ...
    VkResult result = vkCreateXcbSurfaceKHR(context->instance,
                        &create_info, context->allocator, &state->surface);
