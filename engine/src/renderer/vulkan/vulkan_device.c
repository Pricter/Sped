...
b8 vulkan_device_create(vulkan_context* context) {
    ...
    if (available_extension_count != 0) {
        available_extensions = sallocate(sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
        ...
        for (u32 i = 0; i < available_extension_count; ++i) {
            if (strings_equal(available_extensions[i].extensionName, "VK_KHR_portability_subset")) {
                SINFO("Adding required extension 'VK_KHR_portability_subset'.");
                portability_required = true;
                break;
            }
        }
    }
    sfree(available_extensions, sizeof(VkExtensionProperties) * available_extension_count, MEMORY_TAG_RENDERER);
...
void vulkan_device_destroy(vulkan_context* context) {
    ...
    SINFO("Destroying command pools...");
    vkDestroyCommandPool(
        context->device.logical_device,
        context->device.graphics_command_pool,
        context->allocator);

    SINFO("Destroying logical device...");
    if(context->device.logical_device) {
        vkDestroyDevice(context->device.logical_device, context->allocator);
        context->device.logical_device = 0;
    }

    SINFO("Releasing physical device resources...");
    context->device.physical_device = 0;

    if(context->device.swapchain_support.formats) {
        sfree(context->device.swapchain_support.formats, sizeof(VkSurfaceFormatKHR) *
                context->device.swapchain_support.format_count, MEMORY_TAG_RENDERER);
        context->device.swapchain_support.formats = 0;
        context->device.swapchain_support.format_count = 0;
    }

    if(context->device.swapchain_support.present_modes) {
        sfree(context->device.swapchain_support.present_modes, sizeof(VkPresentModeKHR) *
                context->device.swapchain_support.present_mode_count, MEMORY_TAG_RENDERER);
        context->device.swapchain_support.present_modes = 0;
        context->device.swapchain_support.present_mode_count = 0;
    }

    szero_memory(&context->device.swapchain_support.capabilities, sizeof(context->device.swapchain_support.capabilities));
    ...
    vulkan_swapchain_support_info* out_support_info)
{
    ...
    if(out_support_info->format_count != 0) {
        if(!out_support_info->formats) {
            out_support_info->formats = sallocate(sizeof(VkSurfaceFormatKHR) *
    ...
    if(out_support_info->present_mode_count != 0) {
        if(!out_support_info->present_modes) {
            out_support_info->present_modes = sallocate(sizeof(VkPresentModeKHR) *
...
b8 select_physical_device(vulkan_context* context) {
    ...
    for(u32 i = 0; i < physical_device_count; ++i) {
        ...
        // NOTE: Enable if compute is required
        ...
        b8 result = physical_device_meets_requirements(physical_devices[i], context->surface, &properties, &features, &requirements,
        ...
        if(result) {
            ...
            SINFO("GPU Driver version: %d.%d.%d", VK_VERSION_MAJOR(properties.driverVersion),
                  VK_VERSION_MINOR(properties.driverVersion), VK_VERSION_PATCH(properties.driverVersion));
            SINFO("Vulkan API version: %d.%d.%d", VK_VERSION_MAJOR(properties.apiVersion),
                  VK_VERSION_MINOR(properties.apiVersion), VK_VERSION_PATCH(properties.apiVersion));
            
            for(u32 j = 0; j < memory.memoryHeapCount; ++j) {
                f32 memory_size_gib = (((f32)memory.memoryHeaps[i].size) / 1024.0f / 1024.0f / 1024.0f);
                if(memory.memoryHeaps[i].flags & VK_MEMORY_HEAP_DEVICE_LOCAL_BIT) {
                    SINFO("Local GPU memory, Heap: %d: %.2f GiB", j, memory_size_gib);
                } else {
                    SINFO("Shared System memory, Heap: %d: %.2f GiB", j, memory_size_gib);
                }
            }
    ...
    vulkan_swapchain_support_info* out_swapchain_support)
{
    ...
    if(requirements->discrete_gpu) {
        if(properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
            SINFO("Device is not a discrete GPU, a discrete one is required. Skipping.");
            return false;
        }
    }
    ...
    SINFO("Graphics | Present | Compute | Transfer | Name");
    ...
    if(
        (!requirements->graphics || (requirements->graphics &&
                                     out_queue_info->graphics_family_index != -1)) &&
        (!requirements->present || (requirements->present &&
                                     out_queue_info->present_family_index != -1)) &&
        (!requirements->compute || (requirements->compute &&
                                     out_queue_info->compute_family_index != -1)) &&
        (!requirements->transfer || (requirements->transfer &&
                                     out_queue_info->transfer_family_index != -1))
    ) {
        SINFO("Device meets queue reqiurements.");
        STRACE("Graphics Family Index: %i", out_queue_info->graphics_family_index);
        STRACE("Present Family Index: %i", out_queue_info->present_family_index);
        STRACE("Transfer Family Index: %i", out_queue_info->transfer_family_index);
        STRACE("Compute Family Index: %i", out_queue_info->compute_family_index);
           ...
           out_swapchain_support->present_mode_count < 1)
        {
            if(out_swapchain_support->formats) {
                sfree(out_swapchain_support->formats,
                      sizeof(VkSurfaceFormatKHR)
                      * out_swapchain_support->format_count, MEMORY_TAG_RENDERER);
            }
            if(out_swapchain_support->present_modes) {
                sfree(out_swapchain_support->present_modes,
        ...
        if(requirements->device_extension_names) {
            u32 available_extension_count = 0;
            VkExtensionProperties* available_extensions = 0;
            VK_CHECK(vkEnumerateDeviceExtensionProperties(device, 0,
                                &available_extension_count, 0));
            if(available_extension_count != 0) {
                available_extensions = sallocate(sizeof(VkExtensionProperties) *
                ...
                for(u32 i = 0; i < required_extension_count; ++i) {
                    ...
                    if(!found) {    
                        ...
                        sfree(available_extensions, sizeof(VkExtensionProperties)
        ...
        if(requirements->sampler_anisotropy && !features->samplerAnisotropy) {
            SINFO("Device does not support sampleAnisotropy. skipping.");
            return false;
        }
