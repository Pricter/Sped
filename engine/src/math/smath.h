...
SINLINE quat quat_slerp(quat q_0, quat q_1, f32 percentage) {
    ...
    f32 theta_0 = sacos(dot);
    f32 theta = theta_0 * percentage;
    f32 sin_theta = ssin(theta);
    f32 sin_theta_0 = ssin(theta_0);

    f32 s0 = scos(theta) - dot * sin_theta / sin_theta_0;
...
SINLINE f32 deg_to_rad(f32 degrees) {
    return degrees * S_DEG2RAD_MULTIPLIER;
}

SINLINE f32 rad_to_deg(f32 radians) {
    return radians * S_RAD2DEG_MULTIPLIER;
}