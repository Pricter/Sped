...
b8 create_shader_module(
    ...
    if (!filesystem_read_all_bytes(&handle, &file_buffer, &size)) {
        SERROR("Unable to binary read shader module: %s.", file_name);
        return false;
    }
    ...
    // Shader stage info
    szero_memory(&shader_stages[stage_index].shader_stage_create_info, sizeof(VkPipelineShaderStageCreateInfo));
