    ...
    if (!fence->is_signaled) {
        ...
        switch (result) {
            case VK_SUCCESS:
                ...
                return true;
            case VK_TIMEOUT:
                SWARN("vk_fence_wait - Timed out");
                break;
            case VK_ERROR_DEVICE_LOST:
                SERROR("vk_fence_wait - VK_ERROR_DEVICE_LOST.");
                break;
            case VK_ERROR_OUT_OF_HOST_MEMORY:
                SERROR("vk_fence_wait - VK_ERROR_OUT_OF_HOST_MEMORY.");
                break;
            case VK_ERROR_OUT_OF_DEVICE_MEMORY:
                SERROR("vk_fence_wait - VK_ERROR_OUT_OF_DEVICE_MEMORY.");
                break;
            default:
                SERROR("vk_fence_wait - An unknown error has occurred.");
                break;
        }
    } else {
        return true;
