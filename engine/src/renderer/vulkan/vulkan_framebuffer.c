...
void vulkan_framebuffer_create(
    ...
    out_framebuffer->attachments = sallocate(sizeof(VkImageView) * attachment_count, MEMORY_TAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context* context, vulkan_framebuffer* framebuffer) {
    ...
    if (framebuffer->attachments) {
        sfree(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEMORY_TAG_RENDERER);
