...
void test_manager_run_tests() {
    ...
    for (u32 i = 0; i < count; ++i) {
        ...
        SINFO("Executed %d of %d (skipped %d) %s (%.6f sec / %.6f sec total)", i + 1, count, skipped, status, test_time.elapsed, total_time.elapsed);
    }

    clock_stop(&total_time);

    SINFO("Results: %d passed, %d failed, %d skipped.", passed, failed, skipped);
}